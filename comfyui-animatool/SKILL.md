...
2. 随机图（包含随机 tag / 随机画师 / 随机构图等多维度随机）→ `anima-random-gen` 产出参数 → 校验 → 通过 `comfyui-manager` 执行
3. 仅随机画师（其他由用户指定）→ `danbooru-tags --random 5 --for-prompt --json --compact` → 选 1 个画师 → 组装参数 → 通过 `comfyui-manager` 执行
